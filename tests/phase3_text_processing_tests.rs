use semisearch::search::{SearchEngine, SearchOptions, SearchResult, MatchType};
...
impl TestSearchEngine {
    ...
    fn add_content(&mut self, name: &str, content: &str) {
        self.content_store.insert(name.to_string(), content.to_string());
    }
    
    fn search(&self, query: &str, strategy: Option<&str>, options: &SearchOptions) -> Result<Vec<SearchResult>, anyhow::Error> {
    ...
    fn search_multi_strategy(&self, query: &str, strategies: &[&str], options: &SearchOptions) -> Result<Vec<SearchResult>, anyhow::Error> {
        let mut all_results = Vec::new();
...
fn test_text_processor_comprehensive() {
    ...
    let test_cases = vec![
        ...
        ("Code snippet", "fn main() { let x = 5; println!(\"Hello\"); }"),
        ("Mixed content", "Email: user@example.com, Phone: +1-555-123-4567"),
        ("Unicode text", "Café naïve résumé 中文 العربية"),
        ("Numbers and symbols", "Version 2.1.3 released on 2024-01-15"),
    ];
    ...
    for (description, content) in test_cases {
        ...
        for chunk in &chunks {
            assert!(!chunk.content.is_empty(), "Empty content in chunk for: {}", description);
            assert!(!chunk.tokens.is_empty(), "No tokens generated for: {}", description);
            assert!(chunk.line_number > 0, "Invalid line number for: {}", description);
            assert!(chunk.end_char >= chunk.start_char, "Invalid char positions for: {}", description);
        }
        
        // Test complexity calculation
        let complexity = processor.calculate_complexity(content);
        assert!(complexity >= 0.0 && complexity <= 1.0, "Invalid complexity score for: {}", description);
    }
}
...
fn test_search_engine_strategy_registration() {
    ...
    for strategy in expected_strategies {
        assert!(strategies.contains(&strategy), "Missing strategy: {}", strategy);
    }
    
    // Verify strategy requirements
    for strategy_name in &strategies {
        let requirements = engine.get_strategy_requirements(strategy_name);
        assert!(requirements.is_some(), "No requirements for strategy: {}", strategy_name);
        
        let req = requirements.unwrap();
        assert!(req.min_memory_mb > 0, "Invalid memory requirement for: {}", strategy_name);
    }
}
...
fn test_keyword_search_comprehensive() {
    ...
    // Test exact matches
    let results = engine.search("machine learning", Some("keyword"), &options).unwrap();
    ...
    let results_lower = engine.search("MACHINE LEARNING", Some("keyword"), &options).unwrap();
    assert!(!results_lower.is_empty(), "Should handle case insensitivity");
    
    // Test partial matches
    let results_partial = engine.search("learning", Some("keyword"), &options).unwrap();
    assert!(results_partial.len() >= results.len(), "Partial matches should return more results");
}
    ...
    for typo_query in typo_queries {
        ...
        if !results.is_empty() {
            ...
            for result in &results {
                ...
                assert!(result.score > 0.0 && result.score <= 1.0, "Invalid fuzzy score");
            }
        }
...
fn test_regex_search_patterns() {
    ...
    for (pattern, description) in regex_patterns {
        ...
        assert!(!results.is_empty(), "Regex should match {}: {}", description, pattern);
...
fn test_tfidf_search_ranking() {
    ...
    let documents = vec![
        ("doc1.txt", "machine learning algorithm implementation details"),
    ...
    let results = engine.search("machine learning", Some("tfidf"), &options).unwrap();
...
fn test_multi_strategy_search() {
    ...
    let results = engine.search_multi_strategy("learning", &strategies, &options).unwrap();
    
    assert!(!results.is_empty(), "Multi-strategy search should return results");
    
    // Verify we get results (may be from same or different strategies)
    for result in &results {
        assert!(matches!(result.match_type, MatchType::Keyword | MatchType::Fuzzy));
        assert!(result.score > 0.0);
    }
}
...
fn test_search_options_comprehensive() {
    ...
    assert!(limited_results.len() <= 1, "Should respect max_results limit");
...
fn test_language_detection() {
    ...
    for (code, expected_lang) in language_examples {
        ...
        if !chunks.is_empty() {
            assert_eq!(chunks[0].language_hint, expected_lang.map(|s| s.to_string()),
...
fn test_search_result_scoring_consistency() {
    ...
    for strategy in engine.available_strategies() {
        let results = engine.search("machine learning", Some(strategy), &options).unwrap();
        
        for result in &results {
            assert!(result.score >= 0.0 && result.score <= 1.0, 
                "Invalid score {} for strategy {}", result.score, strategy);
        }
...
fn test_empty_and_edge_cases() {
    ...
    let _results = engine.search(&long_query, Some("keyword"), &options).unwrap();
    ...
    for query in special_queries {
        for strategy in engine.available_strategies() {
            ...
            assert!(results.is_ok(), "Should handle special characters in query: {}", query);
        }
    }
}
...
fn test_search_strategy_requirements() {
    ...
    for strategy_name in engine.available_strategies() {
        ...
        assert!(requirements.min_memory_mb >= 10, "Too low memory requirement for {}", strategy_name);
        assert!(requirements.min_memory_mb <= 1000, "Too high memory requirement for {}", strategy_name);
