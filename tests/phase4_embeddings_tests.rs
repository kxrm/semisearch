use semisearch::core::{EmbeddingConfig, LocalEmbedder, EmbeddingCapability};
use semisearch::search::semantic::{SemanticSearch, SemanticSearchOptions, SemanticReranker};
use semisearch::storage::{Database, ChunkRecord};
...
async fn test_phase4_end_to_end_embeddings() {
    ...
    let results = semantic_search.search("machine learning", &chunks, 5).unwrap();
    ...
    // Test with reranking
    let reranked_results = semantic_search.search_with_reranking(
    ...
    if results.len() >= 2 {
        let sim = LocalEmbedder::similarity(
            &results[0].query_embedding,
            &results[0].chunk_embedding
        );
...
async fn test_empty_vocabulary_handling() {
    ...
    assert!(result.unwrap_err().to_string().contains("vocabulary not built"));
}
...
async fn test_embedding_normalization() {
    ...
    assert!(non_zero_count > 0, "Embedding should have at least some non-zero values");
    
    // Check normalization constraint (L2 norm should be <= 1.0 after normalization)
    let norm: f32 = embedding.iter().map(|x| x * x).sum::<f32>().sqrt();
    assert!(norm <= 1.0, "Normalized embedding should have L2 norm <= 1.0");
}
