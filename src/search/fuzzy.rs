...
impl FuzzySearch {
    ...
    fn calculate_fuzzy_score(&self, query: &str, content: &str, tokens: &[String], options: &SearchOptions) -> f32 {
        let query_lower = if options.case_sensitive { query.to_string() } else { query.to_lowercase() };
        let content_lower = if options.case_sensitive { content.to_string() } else { content.to_lowercase() };
        ...
        let combined_score = (skim_score * 0.4) + (token_score * 0.3) + (edit_score * 0.2) + substring_bonus;
        ...
        for query_token in &query_tokens {
            ...
            for content_token in tokens {
                ...
                if edit_dist <= self.max_edit_distance {
                    let edit_score = 1.0 - (edit_dist as f32 / query_token.len().max(content_token.len()) as f32);
                    best_token_score = best_token_score.max(edit_score);
                }
            }
            
            if best_token_score > 0.2 { // Lower threshold for better matching
...
mod tests {
    ...
    fn test_token_fuzzy_score() {
        ...
        assert!(score1 > score2 || (score1 - score2).abs() < 0.01, "Exact match should score higher than typo");
        assert!(score2 > score3);
    }
    ...
    fn test_combined_score_calculation() {
        ...
        let score = search.calculate_fuzzy_score("machine learning", "machine learning algorithm", &tokens, &options);
    ...
    fn test_very_different_strings() {
        ...
        let chunks = vec![
            create_test_chunk(1, "completely different content"),
        ];
