...
impl RegexSearch {
    ...
    fn calculate_regex_score(&self, regex_match: &RegexMatch, content: &str, _options: &SearchOptions) -> f32 {
        ...
        let line_start_bonus = if regex_match.start == 0 || content.chars().nth(regex_match.start - 1) == Some('\n') {
            0.1
        } else {
            0.0
        };
        ...
        let length_penalty = if match_length < 3 {
            -0.1
        } else {
            0.0
        };
        ...
        (base_score + coverage_score * 0.2 + word_boundary_bonus + line_start_bonus + length_penalty).min(1.0).max(0.0)
    }
    ...
    fn is_word_boundary_match(&self, regex_match: &RegexMatch, content: &str) -> bool {
        ...
        let start_boundary = regex_match.start == 0 || 
            !chars.get(regex_match.start - 1).unwrap_or(&' ').is_alphanumeric();
    ...
    pub fn search_wildcard(&self, pattern: &str, chunks: &[TextChunk], options: &SearchOptions) -> Result<Vec<SearchResult>> {
...
mod tests {
    ...
    fn test_whole_words_option() {
        ...
        assert_eq!(results.len(), 1, "Expected 1 result but got {}", results.len());
        assert_eq!(results[0].line_number, 2);
        
        // Verify it doesn't match partial words
        let results_no_whole_words = search.search_chunks("test", &chunks, &SearchOptions::default()).unwrap();
        assert!(results_no_whole_words.len() >= results.len(), "Without whole_words should find same or more matches");
    }
    ...
    fn test_complex_regex_pattern() {
        ...
        let results = search.search_chunks(email_pattern, &chunks, &options).unwrap();
    ...
    fn test_regex_match_positions() {
        ...
        let chunks = vec![
            create_test_chunk(1, "The word test appears here"),
        ];
    ...
    fn test_multiple_matches_per_line() {
        ...
        let chunks = vec![
            create_test_chunk(1, "test and test and test"),
        ];
