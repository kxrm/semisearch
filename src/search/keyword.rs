...
impl KeywordSearch {
    ...
    pub fn search_chunks(&self, query: &str, chunks: &[TextChunk], options: &SearchOptions) -> Result<Vec<SearchResult>> {
        ...
        for chunk in chunks {
            ...
            if score >= options.min_score {
                ...
                let (start_char, end_char) = self.find_match_position(&chunk.content, &query_tokens);
    ...
    fn calculate_keyword_score(&self, query_tokens: &[String], content_tokens: &[String], options: &SearchOptions) -> f32 {
        ...
        for query_token in query_tokens {
            if options.case_sensitive {
                ...
                score += if content_tokens.contains(query_token) { 1.0 } else { 0.0 };
            } else {
                ...
                if content_tokens_set.contains(query_token) {
                    ...
                    for content_token in content_tokens {
                        if content_token.contains(query_token) || query_token.contains(content_token) {
                            score += 0.5;
                            break;
                        }
                    }
    ...
    fn calculate_whole_word_bonus(&self, query_tokens: &[String], content_tokens: &[String]) -> f32 {
        let exact_matches = query_tokens.iter()
...
mod tests {
    ...
    fn test_multi_word_search() {
        ...
        let results = search.search_chunks("machine learning", &chunks, &options).unwrap();
