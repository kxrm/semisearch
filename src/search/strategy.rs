...
impl SearchHelper {
    ...
    pub fn calculate_relevance_score(
        ...
        let length_penalty = if content_length > 1000 {
            0.9
        } else {
            1.0
        };
    ...
    pub fn highlight_matches(content: &str, start: usize, end: usize, highlight_tag: &str) -> String {
...
mod tests {
    ...
    fn test_calculate_term_overlap() {
        ...
        let content_terms1 = vec!["machine".to_string(), "learning".to_string(), "algorithm".to_string()];
    ...
    fn test_add_context() {
        ...
        assert_eq!(results[0].context_before.as_ref().unwrap(), &vec!["Line 2".to_string()]);
        assert_eq!(results[0].context_after.as_ref().unwrap(), &vec!["Line 4".to_string()]);
    }
