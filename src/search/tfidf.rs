use super::{SearchStrategy, SearchOptions, SearchResult, MatchType, ResourceRequirements};
...
impl TfIdfSearch {
    ...
    pub fn search_chunks(&self, query: &str, chunks: &[TextChunk], options: &SearchOptions) -> Result<Vec<SearchResult>> {
        ...
        for chunk in chunks {
            ...
            if score >= options.min_score {
                let (start_char, end_char) = self.find_best_match_position(&chunk.content, &query_tokens);
    ...
    fn calculate_tf(&self, term: &str, term_frequency: &FxHashMap<String, usize>, _document_length: f32) -> f32 {
...
mod tests {
    ...
    fn test_tfidf_scoring() {
        ...
        let results = search.search_chunks("machine learning", &chunks, &SearchOptions::default()).unwrap();
    ...
    fn test_phrase_bonus() {
        ...
        let doc1_tokens = vec!["machine".to_string(), "learning".to_string(), "algorithm".to_string()];
        let doc2_tokens = vec!["learning".to_string(), "machine".to_string(), "algorithm".to_string()];
    ...
    fn test_index_stats() {
        ...
        assert!(stats.most_common_terms.iter().any(|(term, _)| term == "common"));
    }
    ...
    fn test_empty_query_handling() {
        ...
        search.build_index(&chunks).unwrap();
        let results = search.search_chunks("", &chunks, &SearchOptions::default()).unwrap();
    ...
    fn test_no_index_handling() {
        ...
        let results = search.search_chunks("test", &chunks, &SearchOptions::default()).unwrap();
    ...
    fn test_score_normalization() {
        let mut search = TfIdfSearch::new();
        let chunks = vec![
            create_test_chunk(1, "test content with multiple test occurrences"),
        ];
    ...
    fn test_partial_phrase_matching() {
        ...
        let query_tokens = vec!["machine".to_string(), "learning".to_string(), "algorithm".to_string()];
        let window1 = vec!["machine".to_string(), "learning".to_string(), "system".to_string()]; // 2/3 match
        let window2 = vec!["deep".to_string(), "learning".to_string(), "algorithm".to_string()]; // 2/3 match
        let window3 = vec!["computer".to_string(), "vision".to_string(), "system".to_string()]; // 0/3 match
