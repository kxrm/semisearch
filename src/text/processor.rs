...
impl TextProcessor {
    ...
    pub fn process_file_with_overlap(&self, content: &str, chunk_size: usize, overlap: usize) -> Vec<TextChunk> {
    ...
    pub fn calculate_complexity(&self, text: &str) -> f32 {
        ...
        let avg_word_length: f32 = tokens.iter().map(|t| t.len() as f32).sum::<f32>() / tokens.len() as f32;
...
mod tests {
    ...
    fn test_process_file_with_overlap() {
        ...
        if chunks.len() > 1 {
            ...
            assert!(has_overlap, 
                "First chunk words {:?} should overlap with second chunk words {:?}", first_words, second_words);
        }
    }
} 